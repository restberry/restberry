...
RestberryObj.prototype.__isAuthorized = function(next, onError) {
    ...
    if (User) {
        self.model.getFieldNamesOfModel(User, function(fieldNames) {
            ...
            var user = self.restberry.waf && self.restberry.waf.getUser() || null;
        ...
        if (self._preRemove) {
            ...
            utils.forEachAndDone(list, function(preRemove, iter) {
                ...
                self.__preRemove(function(err) {
                    if (err)  return self.onError(errors.BadRequest, err, onError);
                    iter();
                });
            }, function() {
                _remove();
...
RestberryObj.prototype.toJSON = function(next, onError) {
    if (!_.isFunction(next))  throw Error('next is not defined.');
    var self = this;
    self.isAuthorized(function() {
        self.toObject(function(d) {
            ...
            if (!self.href || self.restberry.disableHref)  delete dd['href'];
...
RestberryObj.prototype.toObject = function(next) {
    ...
    _.each(_.keys(obj), function(key) {
        if (!_.contains(fields, key))  delete obj[key];
    });
    ...
    self._populate ? self._populate(populate) : populate();
};

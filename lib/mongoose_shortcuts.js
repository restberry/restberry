    ...
    populate: function(req, res, fieldName, next) {
        ...
        self.constructor.getParents(function(parents) {
            utils.forEachAndDone(parents, function(parent, iter) {
                var obj = utils.dotGet(self, parent.fieldName);
                if (!obj || obj === self) {
                    iter();
                    return;
                }
                var d = [];
                var objs = obj;
                if (!util.isArray(objs))  objs = [objs];
                utils.forEachAndDone(objs, function(objId, iter) {
                    parent.model.findByIdAndVerify(req, res, objId, function(obj) {
                        var fieldName = parent.fieldName.split('.').pop();
                    ...
                    populatedObjs[parent.fieldName] = d;
                    iter();
                });
            }, function() {
                next(populatedObjs);

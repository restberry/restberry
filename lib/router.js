...
Route.prototype._applyController = function() {
    ...
    self.action = function(req, res, next) {
        var action;
        if (req.query.action) {
            action = self.actions ? self.actions[req.query.action] : null;
            if (!action) {
...
Route.prototype.config = function(config) {
    ...
    if (!_.isUndefined(config.isLoginRequired)) {

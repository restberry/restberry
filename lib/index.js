...
var crudRoutes = require('./router-crud');
...
Restberry.prototype.listen = function(name, port, next) {
    ...
    return _listenArgument(name, port, next, function(name, port, next) {
        if (name) {
            self.name = name;
        }
        if (port) {
        ...
        return self.waf.listen(self.waf.port, function() {
            if (self.env) {
...
Restberry.prototype.setDefaultODM = function() {
    var self = this;
    ...
    if (this.verbose) {
        logger.warn('using default ODM (mongoose)');
    }
};

Restberry.prototype.setDefaultWAF = function() {
    this.use('express');
    if (this.verbose) {
        logger.warn('using default WAF (express)');
    }
};
...
var _listenArgument = function(name, port, next, callback) {
    ...
    return callback(name, port, next);
};
